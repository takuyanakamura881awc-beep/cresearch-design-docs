...
def _report_decay(paths: tuple[IntradayPath, ...]) -> None:
    ...
    **ただし実測の形は「なだらかな減衰」ではなく「崖」だった**
    （意思決定ログ86）: 0分 -0.2bps → 5分 -18.7bps → 15分 -22.1 →
    30分 -13.7 → 60分 -13.0。5分で落ちた後は横ばいで、**遅延そのものは
    問題ではない**。問題は**板寄せの約定値を取れるかどうか**の一点。

    板寄せは単一価格で約定するので、その直後に実勢へ寄る。
    ギャップ・フェードが日足で捉えていた優位の正体は、その補正だった。

    **取るには寄成注文が要り、寄成には寄り前にシグナルが確定している
    必要がある。** ギャップは定義上その日の始値で決まるので循環している。
    """
    ...
    print()
    print("  **見るのは「なだらかに減るか」ではなく「0分と5分の間に崖があるか」。**")
    print("  実測（意思決定ログ86）では 0分 -0.2bps → 5分 -18.7bps と落ちた後、")
    print("  15/30/60分はほぼ横ばいだった。**遅延そのものは問題ではなく、")
    print("  板寄せの約定値を取れるかどうかが分かれ目**だった。")
    print()
    print("  → **崖がある** → その優位は板寄せの価格そのもの。")
    print("    寄成注文でしか取れないので、**シグナルが前日引けまでの情報だけで")
    print("    決まる**必要がある。当日の始値を要るシグナルは循環していて取れない")
    print("  → **崖がない（全体になだらか or 平坦）** → 執行速度は成否を分けない。")
    print("    この制約からは自由に設計できる")
