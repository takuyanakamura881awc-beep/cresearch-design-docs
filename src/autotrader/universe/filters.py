...
**想定と食い違ったときに原因を特定できない**。
流動性が厳しすぎるのか、株価レンジが狭すぎるのかで打つ手が変わる。

実測（2026-08-23）で実際に効いた: 通過287銘柄に対し、内訳から
流動性664・株価上限502が絞り込みの主役だと分かった（docs/03-universe.md §1）。
...
DEFAULT_MIN_AVG_TURNOVER_YEN = Decimal(300_000_000)
"""20日平均売買代金の下限。**実測で10億円から引き下げた（2026-08-23）。**

10億円では株価上限1,250円との組み合わせで55銘柄しか残らず、監視枠50の1.1倍で
Layer 2 の選定が機能しなかった。これは安全装置ではなく品質閾値で、
10〜15万円の注文は3億円銘柄でも売買代金の0.033〜0.050%。板を動かさない。

引き下げたぶんは約定モデル側で厳しくしてある
（`broker/replay.py` は売買代金5億円未満に +5bps を上乗せする）。
"""

DEFAULT_TURNOVER_LOOKBACK_DAYS = 20
DEFAULT_PRICE_HARD_MIN = 300
"""株価の下限。**品質の下限であって、コストの下限ではない。**

かつては「安すぎる銘柄は相対コストが重い」もこの値の根拠に含めていたが、
**コストの判定は Layer 2 の ATR円下限に移した**（`autotrader.tick`）。
往復コストは ``スプレッド円 ÷ ATR円`` で決まるので、株価だけを見ても
高いか安いかは判定できない。300円の銘柄でも ATR が30円あれば
1,250円で ATR 12円の銘柄より安い。

ここに残っているのは値付けの粗さと仕手性の回避という別の理由。
"""

DEFAULT_PRICE_NORMAL_MAX = 1000
"""通常枠の上限。**`max_concurrent = 5` から導出した値。**

レバレッジ1倍で5銘柄同時に持つには 1銘柄 ≤ 100% / 5 = 20% = 100,000円。
単元100株なので株価1,000円。ここを超えると同時保有が4銘柄に減る。
"""

DEFAULT_PRICE_PREMIUM_MAX = 1250
"""プレミアム枠の上限 = 取引可能な最大株価。**1銘柄あたり25%上限から導出。**

50万円 × 25%（docs/05-risk-management.md #7）÷ 100株 = 1,250円。
これを超える銘柄は**1単元すら建てられない**ので、ユニバースに入れる意味がない
（選定を通ってもサイジングで0株になり、エラーにならず静かに機会を失う）。

当初の3,000円は資金50万円では実現できないと実測で確定した。
資金が増えたとき真っ先に緩められるのがここ（300万円なら7,500円まで）。
"""
...
class RejectReason(Enum):
    ...
    """C: 1,250円超。1単元が資金の25%を超え、1銘柄あたりの上限（#7）に抵触する"""
    ILLIQUID = "illiquid"
...
def classify_price_tier(
    ...
    **境界は分布ではなく安全装置から導出してある**（docs/03-universe.md §2）::
    ...
    枠                      1単元           扱い
    ======================  ==============  ==================================
    NORMAL 300〜1,000円     3万〜10万円     5銘柄同時に持てる。通常採用
    PREMIUM 1,000〜1,250円  10万〜12.5万円  4銘柄に減る。優位がある場合のみ
    ======================  ==============  ==================================
    ...
    Returns:
        ...
        - ``premium_max`` 超: **1単元が資金の25%を超え、1単元すら建てられない**
    """
    if price < hard_min:
        return None
    if price <= normal_max:
