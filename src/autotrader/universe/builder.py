...
実測（2026-08-23 / 基準日 2026-05-29。``scripts/measure_universe.py``）::
    ...
    全上場 4,451
      ...
      → かつ 貸借銘柄         1,483   （D）
      → ストップ高安を除く    1,476   （H）
      → 株価 300〜3,000円       951   （C: 除外525のうち502が上限超）
      → 売買代金10億円以上      287   （B: 664除外）← Layer 1 の出力

設計時の見込みは100〜200だったが実測287。**外れたのは見込みのほう**なので
閾値は変えていない（docs/03-universe.md §1）。
絞り込みの主役は流動性（664）と株価上限（502）の2つ。
